...
occursion_model = "model/top_formal.ml"
...
bottom_occursion_model = "model/bottom_formal.ml"
bottom_occursion_predictor = classify.make_occursion_model()
bottom_occursion_predictor.add(Dropout(0.0))
bottom_occursion_predictor.load_weights(bottom_occursion_model)
...
def predict_occursion_bottom(image_path, gender):
    predictor = bottom_occursion_predictor
    img = Image.open(image_path).convert('RGB') 
    img = img.resize( (classify.OCASSION_IMAGE_SIZE,classify.OCASSION_IMAGE_SIZE), Image.ANTIALIAS)
    img.save(image_path+'.jpg')
    x = load_img(image_path+'.jpg')
